...
print("vui lòng chờ 1 đến 2 phút để getkey ạ!")
...
# Kiểm tra nếu script được chạy từ dòng lệnh với tham số
if __name__ == "__main__":

    # Kiểm tra nếu có đủ tham số
    if len(sys.argv) < 3:
        print("Vui lòng cung cấp số điện thoại và số lần spam. Ví dụ: python sms.py 0901234567 5")
        sys.exit(1)

    # Lấy số điện thoại và số lần spam từ dòng lệnh
    phone = sys.argv[1]
    try:
        count = int(sys.argv[2])  # Số lần spam
    except ValueError:
        print("Số lần spam phải là một số hợp lệ.")
        sys.exit(1)

    # Chạy vòng lặp để spam SMS
